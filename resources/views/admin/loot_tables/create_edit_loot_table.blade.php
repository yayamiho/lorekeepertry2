...
<div class="form-group">
    {!! Form::label('Name') !!}
    {!! add_help('This is the name you will use to identify this table internally. This name will not be shown to users and does not have to be unique, but a name that can be easily identified is recommended.') !!}
    {!! Form::text('name', $table->name, ['class' => 'form-control']) !!}
</div>

<div class="form-group">
    {!! Form::label('Display Name') !!}
    {!! add_help('This is the name that will be shown to users, for example when displaying the rewards for doing a prompt. This is for display purposes and can be something more vague than the above, e.g. "A Random Rare Item"') !!}
    {!! Form::text('display_name', $table->getRawOriginal('display_name'), ['class' => 'form-control']) !!}
</div>

<h3>Loot</h3>

<p>These are the potential rewards from rolling on this loot table. You can add items, currencies or even another loot
    table. Chaining multiple loot tables is not recommended, however, and may run the risk of creating an infinite loop.
    @if (!$table->id)
...
<table class="table table-sm" id="lootTable">
    <thead>
        <tr>
            ...
            <th width="10%">Weight
                {!! add_help('A higher weight means a reward is more likely to be rolled. Weights have to be integers above 0 (round positive number, no decimals) and do not have to add up to be a particular number.') !!}
            </th>
        ...
        @if ($table->id)
            @foreach ($table->loot as $loot)
                <tr class="loot-row">
                    ...
                    ? ['Item' => 'Item', 'Pet' => 'Pet', 'Border' => 'Border', 'ItemRarity' => 'Item Rarity', 'Currency' => 'Currency', 'LootTable' => 'Loot Table', 'ItemCategory' => 'Item Category', 'ItemCategoryRarity' => 'Item Category (Conditional)', 'None' => 'None']
                    : ['Item' => 'Item', 'Pet' => 'Pet', 'Border' => 'Border', 'Currency' => 'Currency', 'LootTable' => 'Loot Table', 'ItemCategory' => 'Item Category', 'None' => 'None'],
                    ...
                    <td class="loot-row-select">
                        ...
                        @elseif($loot->rewardable_type == 'Pet')
                            ...
                            @elseif($loot->rewardable_type == 'Borders')
                            {!! Form::select('rewardable_id[]', $borders, $loot->rewardable_id, ['class' => 'form-control border-select selectize', 'placeholder' => 'Select Borders']) !!}
                    ...
                    <td class="loot-row-weight">
                        {!! Form::text('weight[]', $loot->weight, ['class' => 'form-control loot-weight']) !!}</td>
...
<div id="lootRowData" class="hide">
    ...
    ? ['Item' => 'Item', 'ItemRarity' => 'Item Rarity', 'Currency' => 'Currency', 'Border' => 'Border', 'Pet' => 'Pet', 'LootTable' => 'Loot Table', 'ItemCategory' => 'Item Category', 'ItemCategoryRarity' => 'Item Category (Conditional)', 'None' => 'None']
    : ['Item' => 'Item', 'Currency' => 'Currency', 'Border' => 'Border', 'Pet' => 'Pet', 'LootTable' => 'Loot Table', 'ItemCategory' => 'Item Category', 'None' => 'None'],
    null,
    ['class' => 'form-control reward-type', 'placeholder' => 'Select Reward Type'],
) !!}</td>
                <td class="loot-row-select"></td>
                <td>{!! Form::text('quantity[]', 1, ['class' => 'form-control']) !!}</td>
                <td class="loot-row-weight">{!! Form::text('weight[]', 1, ['class' => 'form-control loot-weight']) !!}
                </td>
                <td class="loot-row-chance"></td>
                <td class="text-right"><a href="#" class="btn btn-danger remove-loot-button">Remove</a></td>
            </tr>
...
@if ($table->id)
    <h3>Test Roll</h3>
    <p>If you have made any modifications to the loot table contents above, be sure to save it (click the Edit button)
        before testing.</p>
    <p>Please note that due to the nature of probability, as long as there is a chance, there will always be the possibility
        of rolling improbably good or bad results. <i>This is not indicative of the code being buggy or poor game
            balance.</i> Be
        cautious when adjusting values based on a small sample size, including but not limited to test rolls and a small
        amount of user reports.</p>
    <div class="form-group">
