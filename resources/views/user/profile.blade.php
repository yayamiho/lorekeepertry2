...
@endif
@if (Auth::check() && Auth::user()->isStaff && $user->settings->strike_count >= 1)
    <div class="alert alert-warning">
        This user has received {{ $user->settings->strike_count }}
        strike{{ $user->settings->strike_count == 1 ? '' : 's' }}.
    </div>
@endif
<h1>
    ...
    {!! $user->displayName !!}
    <a href="{{ url('reports/new?url=') . $user->url }}"><i class="fas fa-exclamation-triangle fa-xs"
            data-toggle="tooltip" title="Click here to report this user."
            style="opacity: 50%; font-size:0.5em;"></i></a>

    @if ($user->settings->is_fto)
        <span class="badge badge-success float-right" data-toggle="tooltip"
            title="This user has not owned any characters from this world before.">FTO</span>
    @endif
</h1>
<div class="mb-4">
    <div class="row">
        <div class="row col-md-6">
            ...
            <div class="col-md-10 col-8" style="padding-right:0">{!! $user->displayAlias !!}</div>
        </div>
        <div class="row col-md-6">
            <div class="col-md-2 col-4" style="padding-right:0">
                <h5>Joined</h5>
            </div>
            <div class="col-md-10 col-8" style="padding-right:0">{!! format_date($user->created_at, false) !!}
        ...
        <div class="row col-md-6">
            ...
            <div class="col-md-10 col-8" style="padding-right:0">{!! $user->rank->displayName !!}
                {!! add_help($user->rank->parsed_description) !!}
            </div>
        </div>
        @if ($user->birthdayDisplay && isset($user->birthday))
            <div class="row col-md-6">
                ...
                <div class="col-md-10 col-8" style="padding-right:0">{!! $user->birthdayDisplay !!}</div>
            </div>
        @endif
    </div>
</div>
...
@if(count($characters->take(4)->get()->chunk(4)))
    @foreach ($characters->take(4)->get()->chunk(4) as $chunk)
        <div class="row mb-4">
            @foreach ($chunk as $character)
                <div class="col-md-3 col-6 text-center">
                    <div>
                        <a href="{{ $character->url }}"><img src="{{ $character->image->thumbnailUrl }}" class="img-thumbnail"
